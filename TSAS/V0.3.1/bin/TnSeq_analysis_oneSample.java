...
//Processing Parameters.txt file
		String lines="",genome_fasta="",gff="",min_hits="",clippings="",capping="",weights="",format="",result_format="",target_seq="";
		...
		{	
			if(lines.length()>0 && lines.charAt(0)!='#')
			{
				...
				else if(result[0].trim().equalsIgnoreCase("Target seq")) 
				{ 
					if(result.length>1) target_seq = result[1].trim().toUpperCase();
					else target_seq = "";
					if(target_seq.equalsIgnoreCase("")) System.out.println("Transposon sequence specificity: Random");
					else
					{
						System.out.println("Transposon sequence specificity: "+target_seq);
					}
				}
			}
		}
...
//Parsing fasta file for genome sequence data...
		...
		if(!target_seq.equals(""))
		{
			if(genome_wide_insertion_sites==0)
				System.out.println("Number of insertion sites for provided sequence couldn't be determined... Assuming random insertion sequence!");
			else
				System.out.println("\nTotal potential (\""+target_seq+"\") insertion sites in genome: "+ genome_wide_insertion_sites);
		}
...
//Identifying hits per gene...
		...
		ArrayList<ArrayList> gene_insertion_sites=new ArrayList<ArrayList>();
		...
		for(int i=0;i<mapped_reads_reps_exp.size();i++)
		{
			ArrayList<Object> treatment_results2 = TnSeq_analysis_oneSample.gene_level_analysis(min_hits,clippings,mapped_reads_reps_exp.get(i),end,start,unique_locations.get(i),hits_per_unique_location.get(i),replicon,geneID,annotation,target_seq,replicon_names,replicon_seqs);
			...
			else
			{
				...
				gene_insertion_sites.add((ArrayList<Integer>) treatment_results2.get(9));
			}
		}
...
//Calculating statistical significance using binomial distribution and adjusting pvalues using BH approach...
		...
		{
			double temp_hold=0.0;
			
			if(!target_seq.equals(""))
			{
				if(genome_wide_insertion_sites==0)
					temp_hold=TnSeq_analysis_oneSample.binomial_essentiality((Integer)GeneSize.get(0).get(i),(ave_unique_locations/(double)genomeSize),(int)ave_uniqueHits_per_gene.get(i).doubleValue());
				else
				{
					if((Integer)gene_insertion_sites.get(0).get(i)<(int)ave_uniqueHits_per_gene.get(i).doubleValue())//if more insertions counted then actual insertion sites
						temp_hold=TnSeq_analysis_oneSample.binomial_essentiality((Integer)gene_insertion_sites.get(0).get(i),(ave_unique_locations/(double)genome_wide_insertion_sites),(Integer)gene_insertion_sites.get(0).get(i));
					else
						temp_hold=TnSeq_analysis_oneSample.binomial_essentiality((Integer)gene_insertion_sites.get(0).get(i),(ave_unique_locations/(double)genome_wide_insertion_sites),(int)ave_uniqueHits_per_gene.get(i).doubleValue());
				}
			}	
			else
				temp_hold=TnSeq_analysis_oneSample.binomial_essentiality((Integer)GeneSize.get(0).get(i),(ave_unique_locations/(double)genomeSize),(int)ave_uniqueHits_per_gene.get(i).doubleValue());
...
private static ArrayList<Object> gene_level_analysis(String min_hits, String clippings,String mapped_reads_exp,ArrayList<Integer> end,ArrayList<Integer> start,ArrayList<String> unique_locations,ArrayList<Integer> hits_per_unique_location,ArrayList<String> replicon,ArrayList<String> geneID,ArrayList<String> annotation,String target_seq,ArrayList<String> replicon_names,ArrayList<String> replicon_seqs) throws Exception
{
	...
	ArrayList<Integer> gene_insertion_sites = new ArrayList<Integer>();//based on provided transposon target sequence... this will now be the assumed gene length
	...
	returnValue.add(gene_insertion_sites);//9
	return returnValue;
}
